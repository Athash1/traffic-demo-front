...
async function handleLogin() {
  try {
    loading.value = true;
    const response = await axios.post<{code: number; msg: string; data: any}>(`${baseURL}/admin/employee/login`, {
      username: loginForm.value.username,
      password: loginForm.value.password
    });
    if (response.data.code == 200) {
      authStore.adminLogin(response.data.data.token, response.data.data)
      ElMessage.success('Login successfully.');
      await router.push('/admin/dashboard/totalAdd')
    } else {
    ...
    <el-card class="login-card">
      ...
      <el-form>
        ...
        <el-form-item>
          ...
          <el-button type="default" @click="goToUserLogin">Switch to User Login</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </div>
